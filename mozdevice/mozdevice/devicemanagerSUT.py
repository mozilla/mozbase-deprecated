...
class DeviceManagerSUT(DeviceManager):
    ...
    def __init__(self, host, port = 20701, retryLimit = 5,
            deviceRoot = None, debugLevel = mozlog.ERROR, **kwargs):
        DeviceManager.__init__(self, debugLevel)
        self.host = host
        self.port = port
        ...
        while retries < retryLimit:
            try:
                ...
                if retries < retryLimit and not self._sock:
                    ...
                    self.logger.info('Could not connect; sleeping for %d seconds.' % sleep_time)
    ...
    def _doCmds(self, cmdlist, outputfile, timeout):
        ...
        if not self._sock:
            try:
                if self._everConnected:
                    self.logger.info("reconnecting socket")
        ...
        for cmd in cmdlist:
            ...
            try:
                ...
                self.logger.debug("sent cmd: %s" % cmd['cmd'])
            except socket.error, msg:
                self._sock.close()
                self._sock = None
                self.logger.error("Remote Device Error: Error sending data"\
                ...
                while not foundPrompt:
                    ...
                    try:
                        ...
                        if select.select([self._sock], [], [], select_timeout)[0]:
                            ...
                            self.logger.debug("response: %s" % temp)
                            timer = 0
                            if not temp:
                                socketClosed = True
    ...
    def pushFile(self, localname, destname, retryLimit = None):
        ...
        self.logger.debug("push returned: %s" % remoteHash)
    ...
    def pushDir(self, localDir, remoteDir, retryLimit = None):
        retryLimit = retryLimit or self.retryLimit
        self.logger.info("pushing directory: %s to %s" % (localDir, remoteDir))
        ...
        for line in data.splitlines():
            if line:
                ...
                try:
                    ...
                    self.logger.error("Unable to parse process list (bug 805969)")
                    self.logger.error("Line: %s\nFull output of process list:\n%s" % (line, data))
    ...
    def fireProcess(self, appname, failIfRunning=False, maxWaitTime=30):
        ...
        self.logger.info("FIRE PROC: '%s'" % appname)

        if (self.processExist(appname) != None):
            self.logger.warn("process %s appears to be running already\n" % appname)
            if (failIfRunning):
        ...
        self.logger.debug("got pid: %s for process: %s" % (pid, appname))
        return pid
    ...
    def launchProcess(self, cmd, outputFile = "process.txt", cwd = '', env = '', failIfRunning=False):
        ...
        """
        if not cmd:
            self.logger.warn("launchProcess called without command to run")
            return None
    ...
    def killProcess(self, appname, forceKill=False):
        if forceKill:
            self.logger.warn("killProcess(): forceKill parameter unsupported on SUT")
        retries = 0
        while retries < self.retryLimit:
            try:
                ...
                retries +=1
                self.logger.warn("try %d of %d failed to kill %s" %
    ...
    def getDirectory(self, remoteDir, localDir, checkDir=True):
        self.logger.info("getting files in '%s'" % remoteDir)
    ...
    def _getRemoteHash(self, filename):
        ...
        self.logger.debug("remote hash returned: '%s'" % data)
        return data
    ...
    def _wait_for_reboot(self, host, port):
        self.logger.debug('Creating server with %s:%d' % (host, port))
        ...
        if data:
            ...
            self.logger.error('Timed out waiting for reboot callback.')
        s.close()
        return data
    ...
    def reboot(self, ipAddr=None, port=30000):
        ...
        self.logger.info("sending rebt command")
        ...
        self.logger.info("rebt- got status back: %s" % status)
    ...
    def uninstallApp(self, appName, installPath=None):
        ...
        status = data.split('\n')[0].strip()
        self.logger.debug("uninstallApp: '%s'" % status)
        if status == 'Success':
            return
    ...
    def uninstallAppAndReboot(self, appName, installPath=None):
        ...
        self.logger.debug("uninstallAppAndReboot: %s" % data)
        return
    ...
    def updateApp(self, appBundlePath, processName=None, destPath=None, ipAddr=None, port=30000):
        ...
        self.logger.debug("updateApp using command: " % cmd)
        ...
        self.logger.debug("updateApp: got status back: %s" % status)
    ...
    def adjustResolution(self, width=1680, height=1050, type='hdmi'):
        ...
        if self.getInfo('os')['os'][0].split()[0] != 'harmony-eng':
            self.logger.warn("unable to adjust screen resolution on non Tegra device")
            return False

        results = self.getInfo('screen')
        parts = results['screen'][0].split(':')
        self.logger.debug("we have a current resolution of %s, %s" % (parts[1].split()[0], parts[2].split()[0]))
        ...
        self.logger.debug("adjusting screen resolution to %s, %s and rebooting" % (width, height))
